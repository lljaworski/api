...
/**
 * Handles GetInvoicesQuery to retrieve paginated list of invoices.
 */
final class GetInvoicesQueryHandler
{
    ...
    #[AsMessageHandler]
    public function __invoke(GetInvoicesQuery $query): InvoiceCollectionDTO
    {
        $offset = ($query->page - 1) * $query->itemsPerPage;
        
        $invoices = $this->invoiceRepository->findActiveInvoices(
            limit: $query->itemsPerPage,
            offset: $offset,
            search: $query->search
        );
        
        $total = $this->invoiceRepository->countActiveInvoices($query->search);
