...
use App\Application\Query\Invoice\GetInvoicesByCustomerQuery;
...
/**
 * Handles GetInvoicesByCustomerQuery to retrieve invoices for a specific customer.
 */
final class GetInvoicesByCustomerQueryHandler
{
    ...
    #[AsMessageHandler]
    public function __invoke(GetInvoicesByCustomerQuery $query): InvoiceCollectionDTO
    {
        $offset = ($query->page - 1) * $query->itemsPerPage;
        
        $invoices = $this->invoiceRepository->findByCustomerId(
            customerId: $query->customerId,
            limit: $query->itemsPerPage,
            offset: $offset
        );
        
        $total = $this->invoiceRepository->countByCustomerId($query->customerId);
